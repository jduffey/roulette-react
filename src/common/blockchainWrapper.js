...
// Sending any transaction from that last address before deploying the contract will
...
async function depositEthForTokens(from, tokenAddress, amount) {
    const signer = provider.getSigner(from);
    const token = new ethers.Contract(tokenAddress, [
...
async function redeemTokensForEth(from, tokenAddress, amount) {
    const signer = provider.getSigner(from);
    const token = new ethers.Contract(tokenAddress, [
